...
class BaseSolver(object):
    ...
    def _setup(self, ):
        ...
        if self.cfg.load_pre:
            print(f'load_precheckpoint from {self.cfg.load_pre}')
            self.load_tuning_state(self.cfg.load_pre)
    ...
    def load_tuning_state(self, path: str,):
        """only load model for load_preand skip missed/dismatched keys
        """
        if path.startswith('http'):
            state = torch.hub.load_state_dict_from_url(path, map_location='cpu')
        else:
