...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        # 打印原始COCO框信息（前3个）
        if len(boxes) > 0:
            print(f"\n=== 图像 {image_id.item()} 原始COCO框 ===")
            for i in range(min(3, len(boxes))):
                print(f"原始框 {i}: {boxes[i]}")
        
        # guard against no boxes via resizing
        boxes = torch.as_tensor(boxes, dtype=torch.float32).reshape(-1, 4)
        
        # COCO中的框是[x,y,width,height]格式，需要转换为[x1,y1,x2,y2]
        # 修复关键错误：确保正确理解COCO原始框格式
        # 正确的转换：
        # 原始COCO格式: [x,y,width,height] -> XYXY格式: [x,y,x+width,y+height]
        x, y, w, h_box = boxes.unbind(-1)
        boxes = torch.stack([x, y, x + w, y + h_box], dim=1)
        
        # 调试转换后的框
        if len(boxes) > 0:
            print(f"转换后的XYXY框:")
            for i in range(min(3, len(boxes))):
                print(f"XYXY框 {i}: {boxes[i].tolist()}")
